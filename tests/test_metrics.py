...
def test_all_unknown():
    ...
    assert metrics.unsupported_rate_over_evaluable(recs) == 0.0  # no assertions at all
    assert _isnan(metrics.unsupported_rate_over_asserted(recs))  # denominator 0
    assert _isnan(metrics.supported_accuracy_over_asserted(recs))
...
def test_correct_but_unsupported_counts_for_accuracy_not_supported_accuracy():
    recs = [_rec("c1", "T1", "T1", None, "Report mentions pT1 elsewhere, no evidence given.")]
    assert metrics.accuracy(recs) == 1.0
    assert metrics.supported_accuracy_over_evaluable(recs) == 0.0
    assert metrics.unsupported_rate_over_evaluable(recs) == 1.0
...
def test_mixed_scenario_all_metrics():
    ...
    assert all_metrics["unsupported_rate_over_evaluable"] == pytest.approx(1 / 4)
    assert all_metrics["unsupported_rate_over_asserted"] == pytest.approx(1 / 3)
    assert all_metrics["supported_accuracy_over_evaluable"] == pytest.approx(1 / 4)
    assert all_metrics["supported_accuracy_over_asserted"] == pytest.approx(1 / 3)
